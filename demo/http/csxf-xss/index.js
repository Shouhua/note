 ...
 * 1. 登陆了银行，然后在另外一个页面有发送银行api转账，这个事后会自动带上银行的cookie
 * 2. 首先存储性XSS攻击成功，在留言板中嵌套了恶意操作的代码
 * 不管怎么说，CSRF的场景就是在用户不知道的情况下使用用户的凭证干坏事，防范也是基于次。
 * CSRF攻击的防范：
 ...
 * 4. 设置Cookie的sameSite(可以防御在其他页面的攻击，set-cookie: name=lily; SameSite=strict)
 */
