...
const { optimizeDeps } = require('./index')
...
function createMissingImporterRegisterFn(server) {
  ...
  let knownOptimized = server._optimizeDepsMetadata && server._optimizeDepsMetadata.optimized
  let currentMissing = {}
  let handle
  ...
  async function rerun(ssr) {
    ...
    try {
      ...
      knownOptimized = newData && newData.optimized
