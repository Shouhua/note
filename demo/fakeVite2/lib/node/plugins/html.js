...
/** Add script to cache */
// htmlProxyMap: config->filePath->[content0, content1]
 ...
 */
 function buildHtmlPlugin(config) {
  const [preHooks, postHooks] = resolveHtmlTransforms(config.plugins) // plugins与dev一致
  ...
  return {
    ...
    async transform(html, id) {
      if (id.endsWith('.html')) {
        ...
        // 2. 处理asssets包括css, css生成import表达式, 其他的放到assetUrls, 再修改assets的url比如是否使用inline base64/或者__VITE_ASSET__开头的url等, 这个内容根dev环境不同
        // script和css node will be removed
        await traverseHtml(html, id, (node) => {
          ...
          if (node.tag === 'script') {
            ...
            if (isPublicFile) {
              // prepareOutDir中将public复制到root中, 这个不同于dev环境
            ...
            if (isModule) {
              ...
              someScriptsAreDefer ||= !isAsync // 因为都是module，async默认使用defer方式加载，如果没有显式async就是defer???
        ...
        if (
          config.build.polyfillModulePreload && // build环境默认为true
    ...
    async generateBundle(options, bundle) {
      ...
      for (const [id, html] of processedHtml) {
        ...
        // TODO 为什么要先写成__VITE_ASSET__，然后再在generateBundle中替换呢？难道是plugins里面统一好做相应的处理
        ...
        if (chunk) { // NOTICE 这里拿到的chunk.code是transfomr中处理html出来的js
          ...
          // NOTICE when not inlined, inject <script> for entry and modulepreload its dependencies
