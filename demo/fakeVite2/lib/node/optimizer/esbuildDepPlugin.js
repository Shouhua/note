...
const { KNOWN_ASSET_TYPES } = require('../constants')
const {
...
const { browserExternalId } = require('../plugins/resolve')
  ...
  return {
    ...
    setup(build) {
      ...
      build.onResolve(
        { filter: /^[\w@][^:]/ },
        async ({ path: id, importer, kind }) => {
          if (moduleListContains(config.optimizeDeps && config.optimizeDeps.exclude, id)) {
            return {
              path: id,
              external: true
            }
          ...
          if (!importer) {
            // console.log(`no import: ${id}, ${JSON.stringify(resolveEntry(id))}`);
          ...
          // console.log(`importttttttttttttter: ${id}, ${importer}`);
      ...
      build.onLoad({ filter: /.*/, namespace: 'dep' }, ({ path: id }) => {
        ...
        // console.log(`esbuild dep plugin: ${id}, ${entryFile}, ${root}`)
        ...
        if (!imports.length && !exports.length) {
          ...
          if (
            data.hasReExports || // export * from blah
            exports.length > 1 ||
            exports[0] !== 'default'
          ) {
